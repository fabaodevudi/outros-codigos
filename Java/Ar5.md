# kkkkvm de eventos da kkkkgq — kkkk53 kkkkho

Este kkkkta consolida os **eventos e sinais** que governam a kkkkwf e a kkkkuh kkkkho, em alinhamento com a [kkkkvl](kkkkvl), o [kkkkva](kkkkva) e os kkkkwu de kkkkgu, retomada e kkkk3w.
...
O kkkkvn de eventos da kkkkgq define:

- **quais eventos/sinais** o front e o kkkku2 trocam com o kkkke4 (ou com a kkkk3l)
- **quem emite** e **quem consome** cada kkkkx9
- **kkkkmn e kkkkx5** associadas (ex.: variável `kkkkgu`, kkkkj0 kkkkvd, `checkpoint_task_key`)
- **comportamento esperado** do engine e do kkkkh0 em cada caso

Não substitui o kkkkvn de **kkkkvo de kkkk55** (dados de formulário, estado da kkkkgq), que permanece em [kkkkva](kkkkva).

---

## Visão geral dos eventos

| kkkkyc / conceito   | Quem emite        | Quem consome       | kkkk7p / doc de referência        |
| -------------------- | ------------------- | -------------------- | --------------------------------- |
| **Voltar (macro)** | kkkkra → kkkkqa   | kkkkh0 (mensagem)     | kkkkhk-DEC-005, Preservação estado |
| **Retomar**        | kkkkmf / kkkkxv | kkkkqa → Engine   | JORNADA-DEC-001                 |
| **kkkkuz**        | kkkkv7 / kkkkho     | kkkkmf (link)     | JORNADA-DEC-001                 |
| **kkkkyl**     | Engine / kkkkh0      | kkkkqa, kkkkra     | JORNADA-DEC-001, kkkkhk-DEC-005   |
| **kkkkem**  | kkkkh0               | kkkkvs kkkkhj     | CONTRATO_VARIAVEIS, kkkkw9  |

---

## Tipo de kkkkx9 kkkkhk utilizado

A kkkksk utiliza **kkkk3y kkkkwl** para kkkku0 externa com o kkkke4.

**Signals** não são utilizados para kkkkwf da kkkkgq, pois são broadcast e podem afetar múltiplas instâncias.

Eventos como "kkkkgu" e "retomar" devem ser implementados como **kkkk3y kkkkwl correlacionados à kkkk5h do kkkke4**. Isso evita erros de kkkkwb.
...
O uso de `kkkkco` pode ocorrer internamente no kkkku2 (ex.: para chamadas ao engine), mas não deve ser exposto ao front-end. Isso melhora governança e mantém o kkkkvn estável ante mudanças de implementação do engine.

---

## kkkkwh de vida de eventos
...
Eventos kkkkxm (ex.: kkkkgu, retomar) devem ser tratados de forma **idempotente**.

Se o mesmo kkkkx9 for recebido mais de uma vez para a mesma kkkk5h e kkkkvi, o kkkke4 não deve produzir efeitos duplicados (ex.: usuário clica "Voltar" duas vezes).
...
2. kkkkra envia kkkkx9 de kkkkgu com **destino** (kkkk9q ou etapa).
3. kkkkqa publica **mensagem** para o kkkke4 (ex.: mensagem "kkkkgu" com kkkkmn).
4. kkkkh0 trata a mensagem (ex.: kkkkwk Event), **finaliza a kkkk5h ativa do kkkk55 kkkkhj** em execução (se houver).
...
| **kkkkyl no kkkkhj** | Ao reabrir a kkkkem do kkkkhj, o kkkkh0 repassa o kkkkvi (ex.: variável de entrada com `checkpoint_task_key`) para o kkkkhj ou à UI abrir na kkkk9q correta. |
...
**Definição:** O kkkk1x (ou o kkkkxv) **kkkkdp à kkkkgq** em uma sessão posterior. Não é "kkkkgu" dentro da mesma sessão; é **reabrir a mesma kkkk5h de kkkk55** e levar o usuário à **User kkkk8l ativa** correspondente.
...
- **kkkkxw** (quando aplicável): prazo de expiração do link/kkkkvd (ex.: `kkkkvb`).
...
**Definição:** kkkkvq em que o **kkkk38** gera um link (kkkk3w) para o **kkkk1x** continuar a kkkkp3 de onde estiver. O kkkk1x acessa o link e **retoma a mesma kkkk5h de kkkk55** associada à kkkk3l (não se cria nova kkkk5h).

**kkkkvm específico:**

| Campo | Descrição |
| ------- | ----------- |
| `kkkkej` | kkkkxw de retomada (incluído no link). |
| `kkkkvb` | Data/hora de expiração do link. |
| Regra de kkkkag | Retomada negada se kkkk3l encerrada ou expirada (ex.: kkkkyo), mesmo com kkkkvd válido. |

kkkkvq técnico detalhado e kkkkx5 de expiração estão no kkkk7p JORNADA-DEC-001.
...
**Regra:** Usar o identificador da User kkkk8l no kkkkhk como kkkkvi, padronizado no kkkkvn como **`checkpoint_task_key`** (não um "step" numérico nem nome de tela). Assim:
...
**Definição:** O kkkke4 orquestra a kkkkgq iniciando kkkk0n via **kkkkem**. Cada kkkkhj recebe kkkkvo de entrada e devolve kkkkvo de saída; **não mantém estado navegacional** entre invocações.

**Relação com os eventos:**

- **Voltar:** O kkkkh0 pode **cancelar** a kkkkem em execução (kkkkwk Event) e **iniciar outra** kkkk65 (kkkk55 kkkkhj de etapa anterior) com kkkkvo de entrada que representem o kkkkvi.
...
- **kkkkyl:** Pode ser a kkkk9q ativa **no kkkkh0** ou a kkkk9q ativa **dentro do kkkk55 kkkkhj** em execução; o kkkku2 deve resolver a kkkk5h e a kkkk9q corretas (ex.: via API do engine).

kkkkvm de kkkkvo de entrada/saída, ciclo de vida e kkkkwa de kkkkwb: [kkkkva](kkkkva), [kkkkvc](kkkkvc).

---

## Resumo do kkkkvn de kkkkmn

| kkkkyc / conceito | Payload / dados chave |
| ------------------- | ------------------------ |
...
| **kkkkuz**       | kkkkej, kkkkvb; mesma estrutura de retomada + kkkkx5 de expiração da kkkk3l. |
| **kkkkyl**    | `checkpoint_task_key` (ID da User kkkk8l no kkkkhk; não step numérico nem nome de tela). |
...
| Documento | Uso |
| ----------- | ----- |
| [kkkkvl](kkkkvl) | Limites de kkkkyr, modelo de execução, correlação |
| [kkkkva](kkkkva) | Variáveis kkkkh0/kkkkhj, kkkk8i Contract Pattern |
| [kkkkvc](kkkkvc) | Stateless, ciclo de vida, kkkkvx incremental |
